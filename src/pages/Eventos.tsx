...
const fetchEvents = async (): Promise<Event[]> => {
  const { data, error } = await supabase
    .from("events")
    .select("*")
    .order("event_date", { ascending: false }); // Mostra os mais recentes primeiro
    ...
    <section className="w-full py-12 md:py-24 lg:py-32">
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          <h1 className="text-3xl font-bold tracking-tighter sm:text-5xl">Nossos Eventos</h1>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mt-12">
          {isLoading ? (
            ...
            <p className="col-span-full text-center text-muted-foreground">Nenhum evento encontrado.</p>
          )}
        </div>
      </div>
    </section>
